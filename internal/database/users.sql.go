...
const updateUserRedMembership = `-- name: UpdateUserRedMembership :one
UPDATE users 
...
type UpdateUserRedMembershipParams struct {
	IsChirpyRed bool
	ID          uuid.UUID
}

func (q *Queries) UpdateUserRedMembership(ctx context.Context, arg UpdateUserRedMembershipParams) (User, error) {
	row := q.db.QueryRowContext(ctx, updateUserRedMembership, arg.IsChirpyRed, arg.ID)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
